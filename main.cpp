...
class Student:public Person {
    ...
    public:
        ...
        // Parameterized Constructor
        Student(string lName, string fName, int a, happinessLevelLife hLevel, string sID, char sGrade, int sTestScore):Person(lName, fName, a, hLevel) {
            studentID = sID;
            studentGrade = sGrade;
            studentTestScore = sTestScore;
        }
        // Setter Function
        void setStudentInformation(string lName, string fName, int a, happinessLevelLife hLevel, string sID, char sGrade, int sTestScore);
        // Getter Functions
...
// For Unit Testing
    ...
    TEST_CASE("Person default Constructor Testing")
    {
        Person p;
        CHECK(p.getFirstName() == " ");
        CHECK(p.getLastName() == " ");
        CHECK(p.getAge() == 0);
        CHECK(p.getHappinessLevel() == NEUTRAL);
    }

    TEST_CASE("Person parameterized Constructor Testing")
    {
        Person p ("Doe", "John", 40, SAD);
        CHECK(p.getFirstName() == "John");
        CHECK(p.getLastName() == "Doe");
        CHECK(p.getAge() == 40);
        CHECK(p.getHappinessLevel() == SAD);
    }

    TEST_CASE("Student Setter Testing based on Base Class")
    {
        Student s;
        s.setStudentInformation("Smith", "Jane", 22, HAPPY, "123456", 'A', 80);
        // Base Class Members
        CHECK(s.getFirstName() == "Jane");
        CHECK(s.getLastName() == "Smith");
        CHECK(s.getAge() == 22);
        CHECK(s.getHappinessLevel() == HAPPY);
        // Derived Class Members
        CHECK(s.getStudentID() == "123456");
        CHECK(s.getStudentGrade() == 'A');
        CHECK(s.getStudentTestScore() == 80);
    }

    TEST_CASE("Student feedback Testing") 
    {
        Student s1("Ali", "Ahmed", 21, HAPPY, "A1", 'A', 95);
        CHECK(s1.getFeedback() == "Excellent work! Keep up the high performance.");

        Student s2("Bob", "Smith", 22, NEUTRAL, "B2", 'C', 72);
        CHECK(s2.getFeedback() == "Fair performance. Focus on improving weaker areas.");

        Student s3("Tom", "Lee", 19, SAD, "C3", 'F', 40);
        CHECK(s3.getFeedback() == "Needs improvement. Consider extra practice and studying.");
    }
    
    TEST_CASE("Composition Class Date Testing")
    {
        Date d1(15, 8, 2024);
        CHECK(d1.getDay() == 15);
        CHECK(d1.getMonth() == 8);
        CHECK(d1.getYear() == 2024);
    ...
    TEST_CASE("Teacher composition works correctly") 
    {
        Teacher t("Brown", "Lisa", 40, HAPPY, "Math", 10, 5, 2018);

        CHECK(t.getFirstName() == "Lisa");
        CHECK(t.getLastName() == "Brown");
        CHECK(t.getTeachingSubject() == "Math");

        Date hire = t.getHireDate();
        CHECK(hire.getDay() == 10);
        CHECK(hire.getMonth() == 5);
        CHECK(hire.getYear() == 2018);
    }
...
int main(){
    ...
    Student student1("Rahman", "Zakaria", 20, HAPPY, "123456", 'A', 94);
...
// Derived Class Setter Function
void Student::setStudentInformation(string lName, string fName, int a, happinessLevelLife hLevel, string sID, char sGrade, int sTestScore) {
    Person::setPersonalInformation(lName, fName, a, hLevel);
    studentID = sID;
    studentGrade = sGrade;
    studentTestScore = sTestScore;
